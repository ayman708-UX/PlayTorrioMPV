...
local user_opts = {
    ...
    audio_track_mbtn_left_command = "script-message playtorrio-audio-menu",
    ...
    sub_track_mbtn_left_command = "script-message playtorrio-subs-menu",
...
--
-- PlayTorrio Custom Menu System
--

-- Forward declarations for functions that reference each other
local pt_menu_disable_bindings
local pt_menu_enable_bindings
local pt_menu_handle_click

local pt_menu = {
    visible = false,
    menu_type = nil,  -- "subs", "audio", "providers"
    items = {},
    selected = 1,
    scroll_offset = 0,
    max_visible = 10,
    ...
    slider_height = 50,
    padding = 12,
    current_provider = nil,  -- for external subs
}

-- External providers storage (loaded from JSON)
local external_providers = {}

local function load_external_providers()
    -- Try multiple paths to find the JSON file
    local paths = {
        mp.command_native({"expand-path", "~~/"}) .. "/playtorrio-subs.json",
        mp.command_native({"expand-path", "~~/playtorrio-subs.json"}),
    }
    
    -- Also try script directory parent
    local script_dir = mp.find_config_file("scripts")
    if script_dir then
        table.insert(paths, 1, script_dir .. "/../playtorrio-subs.json")
    end
    
    for _, subs_file in ipairs(paths) do
        ...
        if file then
            ...
            if content and content ~= "" then
                ...
                if parsed and parsed.providers then
                    ...
                    msg.info("Loaded " .. #external_providers .. " external subtitle provider(s) from " .. subs_file)
                    for i, p in ipairs(external_providers) do
                        msg.info("  Provider " .. i .. ": " .. p.name .. " (" .. #(p.subtitles or {}) .. " subs)")
                    end
                    return
                end
            end
...
local function pt_menu_close()
    pt_menu.visible = false
    pt_menu.menu_type = nil
    pt_menu.items = {}
    pt_menu.selected = 1
    pt_menu.scroll_offset = 0
    pt_menu.current_provider = nil
end

local function pt_menu_get_subtitle_tracks()
    local items = {}
    table.insert(items, { id = 0, title = "Off", hint = "", active = not mp.get_property_bool("sub-visibility", true), type = "track" })
    
    local tracks = mp.get_property_native("track-list", {})
    for _, track in ipairs(tracks) do
        if track.type == "sub" then
            -- Language is the main title, track title goes to hint
            local title = track.lang and string.upper(track.lang) or ("Track " .. track.id)
            local hint = ""
            if track.title then hint = track.title end
            if track.codec then
                local codec = string.upper(track.codec)
                if codec == "SUBRIP" then codec = "SRT" end
                hint = hint .. (hint ~= "" and " · " or "") .. codec
            end
            if track.external then hint = hint .. (hint ~= "" and " · " or "") .. "EXT" end
            
            table.insert(items, {
                id = track.id,
                title = title,
                hint = hint,
                active = track.selected and mp.get_property_bool("sub-visibility", true),
                type = "track"
            })
        end
    end
    
    -- Add external providers as buttons
    if #external_providers > 0 then
        table.insert(items, { type = "divider" })
        for i, provider in ipairs(external_providers) do
            local sub_count = provider.subtitles and #provider.subtitles or 0
            if sub_count > 0 then
                table.insert(items, {
                    type = "provider",
                    key = "provider_" .. i,
    ...
    -- Add divider and settings
    table.insert(items, { type = "divider" })
    table.insert(items, { type = "slider", key = "delay", title = "Delay", min = -10, max = 10, step = 0.1, unit = "s", prop = "sub-delay" })
    table.insert(items, { type = "slider", key = "size", title = "Size", min = 0.5, max = 2.5, step = 0.1, unit = "×", prop = "sub-scale" })
    table.insert(items, { type = "slider", key = "pos", title = "Position", min = 0, max = 150, step = 5, unit = "%", prop = "sub-pos" })
    table.insert(items, { type = "button", key = "reset", title = "Reset Settings" })
    
    return items
end

local function pt_menu_get_provider_subs(provider_index)
    local items = {}
    local provider = external_providers[provider_index]
    if not provider then return items end
    
    -- Back button
    table.insert(items, { type = "button", key = "back", title = "← Back" })
    table.insert(items, { type = "divider" })
    
    -- Provider subtitles
    for i, sub in ipairs(provider.subtitles or {}) do
        table.insert(items, {
            type = "external_sub",
            title = sub.name or ("Subtitle " .. i),
            hint = "",
            url = sub.url,
            sub_name = sub.name
        })
...
local function pt_menu_get_audio_tracks()
    local items = {}
    local tracks = mp.get_property_native("track-list", {})
    ...
    for _, track in ipairs(tracks) do
        if track.type == "audio" then
            ...
            local hint = ""
            if track.title then hint = track.title end
            if track.codec then hint = hint .. (hint ~= "" and " · " or "") .. string.upper(track.codec) end
            if track["demux-channel-count"] then
                local ch = track["demux-channel-count"]
                local ch_str = ch == 2 and "Stereo" or (ch == 6 and "5.1" or (ch == 8 and "7.1" or ch .. "ch"))
                hint = hint .. (hint ~= "" and " · " or "") .. ch_str
            end
...
local function pt_menu_open(menu_type, provider_index)
    pt_menu.menu_type = menu_type
    pt_menu.selected = 1
    pt_menu.scroll_offset = 0
    pt_menu.current_provider = provider_index
    
    if menu_type == "subs" then
        pt_menu.items = pt_menu_get_subtitle_tracks()
    elseif menu_type == "audio" then
        pt_menu.items = pt_menu_get_audio_tracks()
    elseif menu_type == "provider" and provider_index then
        pt_menu.items = pt_menu_get_provider_subs(provider_index)
    end
    
    -- Select current active item
    for i, item in ipairs(pt_menu.items) do
        if item.active then
            pt_menu.selected = i
            break
        end
    ...
    pt_menu.visible = true
    pt_menu.fade = 1
end

local function pt_menu_select()
    local item = pt_menu.items[pt_menu.selected]
    if not item then return end
    
    if item.type == "track" then
        if pt_menu.menu_type == "subs" then
            if item.id == 0 then
                mp.set_property_bool("sub-visibility", false)
            else
                mp.set_property_bool("sub-visibility", true)
                mp.set_property_number("sid", item.id)
            end
        elseif pt_menu.menu_type == "audio" then
            mp.set_property_number("aid", item.id)
        end
        pt_menu_close()
        pt_menu_disable_bindings()
    elseif item.type == "provider" then
        -- Open provider submenu
        msg.info("Opening provider submenu: " .. item.provider_index)
        pt_menu.menu_type = "provider"
        pt_menu.current_provider = item.provider_index
        pt_menu.items = pt_menu_get_provider_subs(item.provider_index)
        pt_menu.selected = 1
        pt_menu.scroll_offset = 0
        request_tick()
        return
    elseif item.type == "external_sub" then
        -- Load external subtitle
        msg.info("Loading external sub: " .. (item.url or "no url"))
        mp.commandv("sub-add", item.url, "select", item.sub_name or "External")
        mp.osd_message("Loading: " .. (item.sub_name or "subtitle"), 2)
        pt_menu_close()
        pt_menu_disable_bindings()
    elseif item.type == "button" and item.key == "back" then
        -- Go back to subs menu
        msg.info("Going back to subs menu")
        pt_menu.menu_type = "subs"
        pt_menu.current_provider = nil
        pt_menu.items = pt_menu_get_subtitle_tracks()
        pt_menu.selected = 1
        pt_menu.scroll_offset = 0
        request_tick()
        return
    elseif item.type == "button" and item.key == "reset" then
        mp.set_property_number("sub-delay", 0)
        mp.set_property_number("sub-scale", 1)
        mp.set_property_number("sub-pos", 100)
        pt_menu.items = pt_menu_get_subtitle_tracks()
        mp.osd_message("Subtitle settings reset", 1.5)
    end
    request_tick()
end

local function pt_menu_adjust(delta)
    local item = pt_menu.items[pt_menu.selected]
    if not item or item.type ~= "slider" then return end
    
    local current = mp.get_property_number(item.prop, 0)
    local new_val = math.max(item.min, math.min(item.max, current + delta * item.step))
    mp.set_property_number(item.prop, new_val)
end

local function pt_menu_navigate(dir)
    if not pt_menu.visible then return false end
    
    if dir == "up" then
        repeat
            pt_menu.selected = pt_menu.selected - 1
            if pt_menu.selected < 1 then pt_menu.selected = #pt_menu.items end
        until pt_menu.items[pt_menu.selected].type ~= "divider"
        
        if pt_menu.selected <= pt_menu.scroll_offset then
            pt_menu.scroll_offset = math.max(0, pt_menu.selected - 1)
        end
        return true
    elseif dir == "down" then
        repeat
            pt_menu.selected = pt_menu.selected + 1
            if pt_menu.selected > #pt_menu.items then pt_menu.selected = 1 end
        until pt_menu.items[pt_menu.selected].type ~= "divider"
        
        if pt_menu.selected > pt_menu.scroll_offset + pt_menu.max_visible then
            pt_menu.scroll_offset = pt_menu.selected - pt_menu.max_visible
        end
        return true
...
local function pt_menu_render(ass)
    if not pt_menu.visible or pt_menu.fade <= 0 then return end
    
    local alpha = pt_menu.fade
    local items = pt_menu.items
    local w = pt_menu.width
    
    -- Calculate content height
    local content_h = 0
    local track_count = 0
    for _, item in ipairs(items) do
        if item.type == "track" or item.type == "provider" or item.type == "external_sub" then
            content_h = content_h + pt_menu.item_height
            track_count = track_count + 1
        elseif item.type == "slider" then
            content_h = content_h + pt_menu.slider_height
        elseif item.type == "button" then
            content_h = content_h + pt_menu.item_height
        elseif item.type == "divider" then
            content_h = content_h + 12
        end
    end
    
    local h = pt_menu.header_height + content_h + pt_menu.padding * 2
    local screen_w = osc_param.playresx
    local screen_h = osc_param.playresy
    local x = screen_w - 40 - w
    local y = screen_h - 75 - h
    
    -- Colors (purple theme)
    local bg_color = "2E0A1A"      -- dark purple bg
    local accent_color = "FF7DC7"  -- light purple
    local primary_color = "DD4E9D" -- purple
    local white = "FFFFFF"
    local gray = "888888"
    local hover_bg = "4D1A3A"
    local active_bg = "DD4E9D"
    
    -- Shadow
    ass:new_event()
    ass:pos(0, 0)
    ass:an(7)
    ass:append(string.format("{\\1c&H000000&\\1a&H%02X&\\bord0\\shad0\\p1}", math.floor((1 - alpha * 0.5) * 255)))
    ass:draw_start()
    ass:rect_cw(x + 5, y + 5, x + w + 5, y + h + 5)
    ass:draw_stop()
    ...
    ass:pos(0, 0)
    ass:an(7)
    ass:append(string.format("{\\1c&H%s&\\1a&H%02X&\\bord0\\shad0\\p1}", bg_color, math.floor((1 - alpha * 0.97) * 255)))
    ass:draw_start()
    ...
    ass:pos(0, 0)
    ass:an(7)
    ass:append(string.format("{\\1c&H%s&\\1a&H%02X&\\bord0\\shad0\\p1}", primary_color, math.floor((1 - alpha) * 255)))
    ass:draw_start()
    ass:rect_cw(x, y, x + w, y + 2)
    ass:draw_stop()
    
    -- Header text
    local header_text = "SUBTITLES"
    if pt_menu.menu_type == "audio" then
        header_text = "AUDIO"
    elseif pt_menu.menu_type == "provider" and pt_menu.current_provider then
        local provider = external_providers[pt_menu.current_provider]
        header_text = provider and provider.name or "EXTERNAL"
    end
    ass:new_event()
    ass:pos(x + pt_menu.padding, y + pt_menu.header_height / 2)
    ass:an(4)
    ass:append(string.format("{\\fn%s\\fs14\\b1\\1c&H%s&\\1a&H%02X&\\bord0\\shad0}", user_opts.font, accent_color, math.floor((1 - alpha) * 255)))
    ass:append(header_text)
    
    -- ESC hint
    ass:new_event()
    ass:pos(x + w - pt_menu.padding, y + pt_menu.header_height / 2)
    ass:an(6)
    ass:append(string.format("{\\fn%s\\fs9\\b0\\1c&H%s&\\1a&H%02X&\\bord0\\shad0}", user_opts.font, gray, math.floor((1 - alpha) * 255)))
    ass:append("ESC")
    
    -- Items
    local iy = y + pt_menu.header_height + pt_menu.padding
    for i, item in ipairs(items) do
        local is_selected = pt_menu.selected == i
        
        if item.type == "divider" then
            ass:new_event()
            ass:pos(0, 0)
            ass:an(7)
            ass:append(string.format("{\\1c&H%s&\\1a&H%02X&\\bord0\\shad0\\p1}", hover_bg, math.floor((1 - alpha * 0.5) * 255)))
            ass:draw_start()
            ass:rect_cw(x + pt_menu.padding, iy + 4, x + w - pt_menu.padding, iy + 5)
            ass:draw_stop()
            iy = iy + 12
            
        elseif item.type == "track" or item.type == "provider" or item.type == "external_sub" then
            -- Hover/active background
            if item.active then
                ass:new_event()
                ass:pos(0, 0)
                ass:an(7)
                ass:append(string.format("{\\1c&H%s&\\1a&H%02X&\\bord0\\shad0\\p1}", active_bg, math.floor((1 - alpha * 0.85) * 255)))
                ass:draw_start()
                ass:rect_cw(x + 6, iy + 2, x + w - 6, iy + pt_menu.item_height - 2)
                ass:draw_stop()
            elseif is_selected then
                ass:new_event()
                ass:pos(0, 0)
                ass:an(7)
                ass:append(string.format("{\\1c&H%s&\\1a&H%02X&\\bord0\\shad0\\p1}", hover_bg, math.floor((1 - alpha * 0.6) * 255)))
                ass:draw_start()
                ass:rect_cw(x + 6, iy + 2, x + w - 6, iy + pt_menu.item_height - 2)
                ass:draw_stop()
            end
            
            -- Checkmark for tracks, arrow for providers
            local title_offset = 0
            if item.active then
                ass:new_event()
                ass:pos(x + pt_menu.padding + 6, iy + pt_menu.item_height / 2)
                ass:an(5)
                ass:append(string.format("{\\fn%s\\fs12\\b1\\1c&H%s&\\1a&H%02X&\\bord0\\shad0}", user_opts.font, white, math.floor((1 - alpha) * 255)))
                ass:append("✓")
                title_offset = 18
            end
            ...
            if #title > 30 then title = string.sub(title, 1, 27) .. "..." end
            ass:new_event()
            ass:pos(x + pt_menu.padding + title_offset + 4, iy + pt_menu.item_height / 2)
            ass:an(4)
            ass:append(string.format("{\\fn%s\\fs12\\b%d\\1c&H%s&\\1a&H%02X&\\bord0\\shad0}", user_opts.font, item.active and 1 or 0, white, math.floor((1 - alpha) * 255)))
            ass:append(title)
            
            -- Hint or arrow for providers
            if item.type == "provider" then
                ass:new_event()
                ass:pos(x + w - pt_menu.padding - 4, iy + pt_menu.item_height / 2)
                ass:an(6)
                ass:append(string.format("{\\fn%s\\fs10\\b0\\1c&H%s&\\1a&H%02X&\\bord0\\shad0}", user_opts.font, accent_color, math.floor((1 - alpha) * 255)))
                ass:append(item.hint .. " →")
            elseif item.hint and item.hint ~= "" then
                ass:new_event()
                ass:pos(x + w - pt_menu.padding - 4, iy + pt_menu.item_height / 2)
                ass:an(6)
                ass:append(string.format("{\\fn%s\\fs9\\b0\\1c&H%s&\\1a&H%02X&\\bord0\\shad0}", user_opts.font, item.active and "CCCCCC" or gray, math.floor((1 - alpha) * 255)))
                ass:append(item.hint)
            end
            
            iy = iy + pt_menu.item_height
        ...
        elseif item.type == "slider" then
            ...
            if is_selected then
                ...
                ass:pos(0, 0)
                ass:an(7)
                ass:append(string.format("{\\1c&H%s&\\1a&H%02X&\\bord0\\shad0\\p1}", hover_bg, math.floor((1 - alpha * 0.4) * 255)))
                ass:draw_start()
                ass:rect_cw(x + 6, iy, x + w - 6, iy + pt_menu.slider_height - 4)
                ass:draw_stop()
            end
            
            -- Label
            ass:new_event()
            ass:pos(x + pt_menu.padding, iy + 13)
            ass:an(4)
            ass:append(string.format("{\\fn%s\\fs10\\b0\\1c&H%s&\\1a&H%02X&\\bord0\\shad0}", user_opts.font, "BBBBBB", math.floor((1 - alpha) * 255)))
            ass:append(item.title)
            
            -- Value
            local val = mp.get_property_number(item.prop, 0)
            local val_str
            if item.key == "delay" then
                val_str = string.format("%+.1f%s", val, item.unit)
            else
                val_str = string.format("%.1f%s", val, item.unit)
            end
            ass:new_event()
            ass:pos(x + w - pt_menu.padding, iy + 13)
            ass:an(6)
            ass:append(string.format("{\\fn%s\\fs10\\b1\\1c&H%s&\\1a&H%02X&\\bord0\\shad0}", user_opts.font, is_selected and accent_color or white, math.floor((1 - alpha) * 255)))
            ass:append(val_str)
            
            -- Slider track
            local sx = x + pt_menu.padding
            local sw = w - pt_menu.padding * 2
            local sy = iy + 30
            local sh = 3
            ...
            ass:pos(0, 0)
            ass:an(7)
            ass:append(string.format("{\\1c&H%s&\\1a&H%02X&\\bord0\\shad0\\p1}", hover_bg, math.floor((1 - alpha * 0.8) * 255)))
            ass:draw_start()
            ass:rect_cw(sx, sy, sx + sw, sy + sh)
            ass:draw_stop()
            ...
            ass:append(string.format("{\\1c&H%s&\\1a&H%02X&\\bord0\\shad0\\p1}", primary_color, math.floor((1 - alpha) * 255)))
            ass:draw_start()
            ass:rect_cw(sx, sy, sx + fill_w, sy + sh)
            ass:draw_stop()
            
            -- Handle
            local hx = sx + fill_w
            local hy = sy + sh / 2
            local hr = is_selected and 5 or 4
            ass:new_event()
            ass:pos(0, 0)
            ass:an(7)
            ass:append(string.format("{\\1c&H%s&\\1a&H%02X&\\bord0\\shad0\\p1}", white, math.floor((1 - alpha) * 255)))
            ass:draw_start()
            local c = 0.55
            ass:move_to(hx, hy - hr)
            ass:bezier_curve(hx + hr*c, hy - hr, hx + hr, hy - hr*c, hx + hr, hy)
            ass:bezier_curve(hx + hr, hy + hr*c, hx + hr*c, hy + hr, hx, hy + hr)
            ass:bezier_curve(hx - hr*c, hy + hr, hx - hr, hy + hr*c, hx - hr, hy)
            ass:bezier_curve(hx - hr, hy - hr*c, hx - hr*c, hy - hr, hx, hy - hr)
            ass:draw_stop()
            
            iy = iy + pt_menu.slider_height
            
        elseif item.type == "button" then
            -- Button background
            local btn_color = is_selected and primary_color or hover_bg
            ass:new_event()
            ass:pos(0, 0)
            ass:an(7)
            ass:append(string.format("{\\1c&H%s&\\1a&H%02X&\\bord0\\shad0\\p1}", btn_color, math.floor((1 - alpha * (is_selected and 1 or 0.5)) * 255)))
            ass:draw_start()
            ass:rect_cw(x + pt_menu.padding, iy + 4, x + w - pt_menu.padding, iy + pt_menu.item_height - 4)
            ass:draw_stop()
            
            -- Button text
            ass:new_event()
            ass:pos(x + w / 2, iy + pt_menu.item_height / 2)
            ass:an(5)
            ass:append(string.format("{\\fn%s\\fs11\\b1\\1c&H%s&\\1a&H%02X&\\bord0\\shad0}", user_opts.font, white, math.floor((1 - alpha) * 255)))
            ass:append(item.title)
...
local pt_menu_bindings_active = false

pt_menu_enable_bindings = function()
    if pt_menu_bindings_active then return end
    pt_menu_bindings_active = true
    
    mp.add_forced_key_binding("UP", "pt-menu-up", function() pt_menu_navigate("up") end, {repeatable=true})
    mp.add_forced_key_binding("DOWN", "pt-menu-down", function() pt_menu_navigate("down") end, {repeatable=true})
    mp.add_forced_key_binding("LEFT", "pt-menu-left", function() pt_menu_navigate("left") end, {repeatable=true})
    mp.add_forced_key_binding("RIGHT", "pt-menu-right", function() pt_menu_navigate("right") end, {repeatable=true})
    mp.add_forced_key_binding("ENTER", "pt-menu-enter", function() pt_menu_navigate("enter") end)
    mp.add_forced_key_binding("ESC", "pt-menu-esc", function() pt_menu_navigate("esc") end)
    mp.add_forced_key_binding("MBTN_LEFT", "pt-menu-click", pt_menu_handle_click)
end

pt_menu_disable_bindings = function()
    if not pt_menu_bindings_active then return end
    pt_menu_bindings_active = false
    
    mp.remove_key_binding("pt-menu-up")
    mp.remove_key_binding("pt-menu-down")
    mp.remove_key_binding("pt-menu-left")
    mp.remove_key_binding("pt-menu-right")
    mp.remove_key_binding("pt-menu-enter")
    mp.remove_key_binding("pt-menu-esc")
    mp.remove_key_binding("pt-menu-click")
end

-- Handle menu mouse click
pt_menu_handle_click = function()
    if not pt_menu.visible then return end
    
    -- Use ModernZ's virtual mouse position function
    ...
    local content_h = 0
    for _, item in ipairs(items) do
        if item.type == "track" then content_h = content_h + pt_menu.item_height
        elseif item.type == "slider" then content_h = content_h + pt_menu.slider_height
        elseif item.type == "button" then content_h = content_h + pt_menu.item_height
        elseif item.type == "divider" then content_h = content_h + 12
        end
    end
    local h = pt_menu.header_height + content_h + pt_menu.padding * 2
    local menu_x = screen_w - 40 - w
    local menu_y = screen_h - 75 - h
    
    -- Check if click is inside menu
    if mouseX >= menu_x and mouseX <= menu_x + w and mouseY >= menu_y and mouseY <= menu_y + h then
        -- Use the currently hovered item (hover updates pt_menu.selected)
        local item = pt_menu.items[pt_menu.selected]
        if item and item.type ~= "divider" then
            if item.type == "slider" then
                -- Click on slider - adjust based on position
                local slider_x = menu_x + pt_menu.padding
                local slider_w = w - pt_menu.padding * 2
                local click_pct = (mouseX - slider_x) / slider_w
                click_pct = math.max(0, math.min(1, click_pct))
                local new_val = item.min + click_pct * (item.max - item.min)
                new_val = math.floor(new_val / item.step + 0.5) * item.step
                mp.set_property_number(item.prop, new_val)
            else
                pt_menu_select()
            end
...
-- Script messages to open menus
mp.register_script_message("playtorrio-subs-menu", function()
    if pt_menu.visible and pt_menu.menu_type == "subs" then
        pt_menu_close()
        pt_menu_disable_bindings()
    else
        pt_menu_close()
        pt_menu_open("subs")
        pt_menu_enable_bindings()
    end
    request_tick()
end)

mp.register_script_message("playtorrio-audio-menu", function()
    if pt_menu.visible and pt_menu.menu_type == "audio" then
        pt_menu_close()
        pt_menu_disable_bindings()
    else
        pt_menu_close()
...
local function hide_osc()
    msg.trace("hide_osc")
    -- PlayTorrio: Don't hide OSC when menu is open
    if pt_menu.visible then
        return
    end
...
local function process_event(source, what)
    ...
    -- PlayTorrio: Handle menu clicks first
    if pt_menu.visible and (what == "down" or what == "up") and source == "mbtn_left" then
        local mouseX, mouseY = get_virt_mouse_pos()
        local screen_w = osc_param.playresx
        local screen_h = osc_param.playresy
        ...
        if mouseX >= menu_x and mouseX <= menu_x + w and mouseY >= menu_y and mouseY <= menu_y + h then
            if what == "up" then
                -- Use the currently hovered/selected item (hover already works)
                local item = pt_menu.items[pt_menu.selected]
                if item and item.type ~= "divider" then
                    if item.type == "track" or item.type == "button" or item.type == "provider" or item.type == "external_sub" then
                        pt_menu_select()
                    elseif item.type == "slider" then
                        -- Click on slider - adjust based on position
                        local sx = menu_x + pt_menu.padding
                        local sw = w - pt_menu.padding * 2
                        local click_pct = (mouseX - sx) / sw
                        click_pct = math.max(0, math.min(1, click_pct))
                        local new_val = item.min + click_pct * (item.max - item.min)
                        new_val = math.floor(new_val / item.step + 0.5) * item.step
                        mp.set_property_number(item.prop, new_val)
            ...
            request_tick()
            return -- Don't process other events when clicking menu
        else
            -- Click outside menu - close it
            if what == "up" then
                pt_menu_close()
    ...
    elseif source == "mouse_move" then
        ...
        -- PlayTorrio: Update menu hover state
        if pt_menu.visible then
            local screen_w = osc_param.playresx
            local screen_h = osc_param.playresy
            local items = pt_menu.items
            ...
            if mouseX >= menu_x and mouseX <= menu_x + w and mouseY >= menu_y and mouseY <= menu_y + h then
                -- Find hovered item
                local iy = menu_y + pt_menu.header_height + pt_menu.padding
                for i, item in ipairs(items) do
                    local item_h = 0
                    if item.type == "track" or item.type == "provider" or item.type == "external_sub" then
                        item_h = pt_menu.item_height
                    elseif item.type == "slider" then item_h = pt_menu.slider_height
                    elseif item.type == "button" then item_h = pt_menu.item_height
                    elseif item.type == "divider" then item_h = 12
                    end
                    
                    if mouseY >= iy and mouseY < iy + item_h and item.type ~= "divider" then
                        pt_menu.selected = i
                        break
                    end
                    iy = iy + item_h
...
local function render()
    ...
    -- PlayTorrio: When menu is open, expand input area to full screen
    if pt_menu.visible then
        ...
        mouse_over_osc = true  -- Keep OSC visible
    else
        for _,cords in ipairs(osc_param.areas["input"]) do
            if state.osc_visible then -- activate only when OSC is actually visible
                set_virt_mouse_area(cords.x1, cords.y1, cords.x2, cords.y2, "input")
            end
            -- PlayTorrio: Keep input enabled when menu is open
            local should_enable = state.osc_visible or pt_menu.visible
            if should_enable ~= state.input_enabled then
                if should_enable then
                    mp.enable_key_bindings("input")
                else
                    mp.disable_key_bindings("input")
                end
                state.input_enabled = should_enable
            end
    ...
    -- PlayTorrio custom menu (rendered on top)
    pt_menu_render(ass)
...
mp.register_event("file-loaded", function()
    ...
    check_path_url()
    load_external_providers()  -- Load external subtitle providers
